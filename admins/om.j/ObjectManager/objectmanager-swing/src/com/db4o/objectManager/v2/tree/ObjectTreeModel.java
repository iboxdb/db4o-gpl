	...
	public int getChildCount(Object parent) {
		...
		if (reflectClass.isArray()) {
			//System.out.println("is array " + parentNode.getObject().getClass());
                        Object to = parentNode.getObject();
                        if ( to instanceof GenericArray){
                           ArrayList al = new ArrayList();
                           Iterator4 ai = (Iterator4)((GenericArray)to).iterator();
                            while( ai.moveNext()){
                                al.add(ai.current());
                            }
                            return al.size();
                        }
			return Array.getLength(to);
...
//			System.out.println("isBoolean: " + (Boolean.class.isAssignableFrom(c)));
	...
	private void addToBatch(Object o) {
		// similar to Object
		session.getObjectContainer().store(o);
