	...
	public partial class dbDataGridView : DataGridView
	{
		...
		public void SetColumnHeaderStyle()
		{
			try
			{
                ...
                cellstyle.SelectionForeColor = Color.IndianRed;
                cellstyle.SelectionBackColor = Color.LightYellow;
		...
		internal void SetDataGridColumnHeader(Hashtable hashColumn, string className, Hashtable hashAttributes)
		{
			try
			{
                ...
                var cc =                Columns[Constants.QUERY_GRID_ISEDITED_HIDDEN];
                //cc.MinimumWidth = 0;
                Columns[Constants.QUERY_GRID_ISEDITED_HIDDEN].Width = 5;
					...
					switch (coltypename)
					{
						...
						default:
                            ...
                            try
                            {
                                ...
                                {
                                    Rows[dataIndex].Cells[colname].Value =
                                    dataObjectValue != null ? dataObjectValue.ToString() : "{NULL}";
                                }
                            }
                            catch (Exception pex)
                            {
                                Rows[dataIndex].Cells[colname].Value =  " {NULL-Value}";
                                //Rows[dataIndex].Cells[colname].Value = pex.ToString();
                            }
