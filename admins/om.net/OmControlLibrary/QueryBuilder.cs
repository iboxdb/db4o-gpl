	...
	public partial class QueryBuilder : ViewBase, ILoadData 
	{
		...
		private void buttonAddQueryGroup_Click(object sender, EventArgs e)
		{
			try
			{
                ...
                {
                    MessageBox.Show("From ObjectBrowser, Right Click FieldName, Add to Query");
                    return;
                }
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void comboboxRecentQueries_DropdownItemSelected(object sender, EventArgs e)
		{
			try
			{
                /*
		...
		private void InitializeRecentQueries()
		{
            ...
            {
                comboboxRecentQueries.Items.Clear();
                var deftxt = Helper.GetResourceString(Constants.COMBOBOX_DEFAULT_TEXT);
                comboboxRecentQueries.Items.Add(deftxt);
                comboboxRecentQueries.SelectedIndex = 0;
                comboboxRecentQueries.SelectedIndexChanged += comboboxRecentQueries_DropdownItemSelected;
                Helper.PopulateRecentQueryComboBox(comboboxRecentQueries);
            }
            catch (Exception oEx)
            {
